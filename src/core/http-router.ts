...
export async function handleHttpRequest(
  ...
  const {
    ...
    versionChecker, deepgramStt, callAgent, processMessage, log,
  } = deps;
  ...
  if (req.method === "GET" && req.url === "/ai-health") {
    ...
    const hasKey = !!(process.env.AI_API_KEY || process.env.CLAUDE_PROXY_API_KEY);
    let apiOk = false;
    let latencyMs: number | undefined;
    if (hasKey && baseUrl) {
      const key = process.env.AI_API_KEY || process.env.CLAUDE_PROXY_API_KEY || "";
  ...
  // GET /stt-status
  if (req.method === "GET" && req.url === "/stt-status") {
    res.writeHead(200, { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" });
    res.end(JSON.stringify({ enabled: deepgramStt?.enabled ?? false }));
    return;
  }
