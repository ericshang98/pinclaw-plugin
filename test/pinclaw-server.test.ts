  ...
  return new Promise((resolve) => {
    mockGatewayServer = createServer(async (req: IncomingMessage, res: ServerResponse) => {
      if (req.method === "POST" && req.url === "/v1/chat/completions") {
        ...
        lastGatewayRequest = JSON.parse(Buffer.concat(chunks).toString());
        ...
        res.end(JSON.stringify({
          id: "mock-001",
          choices: [{
            message: { role: "assistant", content: MOCK_AGENT_REPLY },
            finish_reason: "stop",
          }],
        }));
        return;
...
async function testTextMessage() {
  console.log("\n📋 Test: WebSocket text message → agent response");
  ...
  assertEq(reply.type, "agent_message", "Should reply agent_message");
  assertEq(reply.content, MOCK_AGENT_REPLY, "Should contain mock agent reply");
  assertEq(reply.proactive, false, "Should not be proactive");
  assertEq(gatewayRequestCount, prevCount + 1, "Should have called gateway once");

  // Verify gateway received correct request
  assert(lastGatewayRequest !== null, "Gateway should have received request");
  assertEq(lastGatewayRequest.messages?.[1]?.content, "你好，你是谁？", "Gateway should get user message");
...
async function testConcurrentMessages() {
  ...
  const promises = [];
  for (let i = 0; i < 3; i++) {
    promises.push(sendAndReceive(ws, { type: "text", content: `并发消息 ${i}` }));
  }

  // But since sendAndReceive uses ws.once, we need a different approach
  // Send them sequentially but verify all get responses
  let successCount = 0;
  for (let i = 0; i < 3; i++) {
    const reply = await sendAndReceive(ws, { type: "text", content: `顺序消息 ${i}` });
    if (reply.type === "agent_message" && reply.content === MOCK_AGENT_REPLY) {
      successCount++;
    }
  }
  assertEq(successCount, 3, "All 3 messages should get valid responses");
